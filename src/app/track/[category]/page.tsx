...
const categoryExamples: Record<Category, PlaceExample[]> = {
  ...
  unesco: [
    { name: 'Machu Picchu, Peru', fact: 'Built without mortar - stones fit so precisely a knife blade won\'t fit between them', reason: 'Witness the lost city of the Incas at sunrise through the Sun Gate' },
    { name: 'Great Barrier Reef, Australia', fact: 'The only living structure visible from space', reason: 'Snorkel among 1,500 species of fish and 400 types of coral' },
    { name: 'Petra, Jordan', fact: 'Only 15% of the ancient city has been explored', reason: 'Walk through the Siq canyon to the Treasury at golden hour' },
    { name: 'Angkor Wat, Cambodia', fact: 'Largest religious monument in the world covering 402 acres', reason: 'Watch sunrise silhouette the temple\'s iconic towers' },
    { name: 'Galápagos Islands, Ecuador', fact: 'Inspired Darwin\'s theory of evolution', reason: 'See species found nowhere else on Earth with no fear of humans' },
    { name: 'Taj Mahal, India', fact: 'Changes color throughout the day - pinkish at dawn, white midday, golden at dusk', reason: 'Witness this monument to eternal love at sunrise' },
    { name: 'Venice, Italy', fact: 'Built on 118 small islands connected by over 400 bridges', reason: 'Explore a car-free city where boats are the main transport' },
    { name: 'Serengeti, Tanzania', fact: 'Hosts the largest terrestrial mammal migration on Earth', reason: 'Witness two million wildebeest crossing crocodile-filled rivers' },
  ],
  fiveKPeaks: [
        ...
        <section className="mt-16">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {categoryExamples[category as Category]?.map((example, index) => (
              ...
              >
                <h3 className="font-bold text-gray-800 text-lg mb-2">{example.name}</h3>
                <div className="space-y-2">
                  <p className="text-gray-600 text-sm">
                    <span className="font-medium text-blue-600">Unique fact:</span>{' '}
                    {example.fact}
                  </p>
                  <p className="text-gray-600 text-sm">
                    <span className="font-medium text-purple-600">Why visit:</span>{' '}
                    {example.reason}
                  </p>
                </div>
              </div>
